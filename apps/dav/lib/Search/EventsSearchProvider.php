...
class EventsSearchProvider extends ACalendarSearchProvider {
	 ...
	 * @return string
	 */
	private function getDeepLinkToCalendarApp(string $principalUri,
											  string $calendarUri,
	...
	protected function getDavUrlForCalendarObject(string $principalUri,
		...
		return $this->urlGenerator->getAbsoluteURL(
			$this->urlGenerator->linkTo('', 'remote.php') . '/dav/calendars/'
			. $principalId . '/'
			. $calendarUri . '/'
			. $calendarObjectUri
		);
	 ...
	 * @return Property
	 */
	private function getDTEndForEvent(Component $eventComponent):Property {
